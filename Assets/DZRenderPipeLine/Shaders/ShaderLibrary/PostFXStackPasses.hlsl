...
{
    Varyings output;
    output.posCS = float4(vertexID <= 1 ? -1.0 : 3.0, vertexID == 1 ? 3.0 : -1.0, 0.0, 1.0);
...
float4 BloomHorizontalPassFragment(Varyings input):SV_TARGET
{
    ...
    for (int i = 0; i < 9; i++)
    {
        float offset = offests[i] * 2.0 * GetSourceTexelSize().x;
...
float4 BloomVerticalPassFragment(Varyings input):SV_TARGET
{
    ...
    for (int i = 0; i < 5; i++)
    {
        float offset = offests[i] * 2.0 * GetSourceTexelSize().y;
